...
func count_sequences(data []byte, chr byte) sequence {
    ...
    newScore.score = math.Pow(float64(newScore.average), float64(newScore.max))
    return newScore
}
...
func process_chunk(dat []byte, seed int64, char byte) (ret chunk_data) {
  ...
  for charIndex := 0; charIndex < len(data); charIndex++ {
	  ...
	  if (uint64(rng.Intn(len(data))) > initialScore.total * initialScore.total) {
	      if (rng.Intn(100) > 90) {
	        data[charIndex] = data[charIndex] ^ byte(rng.Intn(255))
	      }
	  }
	}
  ...
  ret.score = newScore
  if (newScore.score > initialScore.score && newScore.total > initialScore.total) {
...
func main() {
    ...
    for true != false {
    	for (i := 0; i < 5; i++) {
	    	...
	    	go func() {
	    		defer(wg.Done)
				newChunk := process_chunk(dat, seed, best_char)
				if newChunk.seed > -1 && newChunk.score.score > bestScore {
					fmt.Printf("\033[2K\rSeed: %d, Score: %+v\n", newChunk.seed, newChunk.score)
